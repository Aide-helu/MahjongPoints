...
public sealed class OnnxHandImageRecognizer : IHandImageRecognizer, IDisposable
{
    ...
    private const string ModelFileName = "shiranai_shallow_best.onnx";
    ...
    {
        _modelPath = FirstExistingOrNull(
            modelPath,
            Environment.GetEnvironmentVariable("MAHJONG_ONNX_MODEL"),
            System.IO.Path.Combine(AppContext.BaseDirectory, "Models", ModelFileName),
            System.IO.Path.GetFullPath(System.IO.Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "Models", ModelFileName)));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(new MahjongHandRecognitionResult(
            tiles,
            _modelPath is null ? ModelFileName : System.IO.Path.GetFileName(_modelPath),
            "ONNX Runtime",
            $"识别完成：{tiles.Length} 张。"));
    }
    ...
    {
        using var source = BitmapFactory.DecodeFile(imagePath)
            ?? throw new InvalidOperationException("Image file cannot be decoded.");
        ...
        using var canvasBitmap = Bitmap.CreateBitmap(InputSize, InputSize, Bitmap.Config.Argb8888!)
            ?? throw new InvalidOperationException("Image buffer cannot be created.");
        using (var canvas = new Canvas(canvasBitmap))
        using (var paint = new Paint { FilterBitmap = true })
        using (var sourceRect = new Rect(0, 0, graySource.Width, graySource.Height))
        using (var targetRect = new Rect(padX, padY, padX + resizedWidth, padY + resizedHeight))
        {
            canvas.DrawColor(Color.Black);
            canvas.DrawBitmap(graySource, sourceRect, targetRect, paint);
        }
        ...
        for (var y = 0; y < InputSize; y++)
        {
            ...
            {
                var value = Color.GetRedComponent(canvasBitmap.GetPixel(x, y)) / 255f;
    ...
    private static Bitmap CreateAutocontrastGrayImage(Bitmap source)
    {
        ...
        for (var y = 0; y < source.Height; y++)
        {
            for (var x = 0; x < source.Width; x++)
            {
                ...
                var value = (byte)Math.Clamp(
                    (int)MathF.Round(
                        Color.GetRedComponent(pixel) * 0.299f +
                        Color.GetGreenComponent(pixel) * 0.587f +
                        Color.GetBlueComponent(pixel) * 0.114f),
                    0,
                    255);
        ...
        var bitmap = Bitmap.CreateBitmap(source.Width, source.Height, Bitmap.Config.Argb8888!)
            ?? throw new InvalidOperationException("Image buffer cannot be created.");
        ...
        for (var y = 0; y < source.Height; y++)
        {
            for (var x = 0; x < source.Width; x++)
            {
                ...
                bitmap.SetPixel(x, y, Color.Rgb(stretched, stretched, stretched));
            }
        }
    ...
    private InferenceSession CreateSession()
    {
        ...
        using var stream = AssetLoader.Open(new Uri(
            $"avares://{typeof(OnnxHandImageRecognizer).Assembly.GetName().Name}/Models/{ModelFileName}"));
        using var memory = new MemoryStream();
        stream.CopyTo(memory);
        return new InferenceSession(memory.ToArray());
    }

    private static string? FirstExistingOrNull(params string?[] values)
    {
